...
export class BlogCategory implements OnInit {
  ...
  private loadBlogs(): void {
    ...
    if (this.specialtyName) {
      keywordsParam = this.specialtyName;
    } else {
      keywordsParam = this.subcat ? this.subcat : (cat && cat.keywords.length > 0 ? cat.keywords.join(',') : '');
    }

    const kwQuery = keywordsParam ? `&keywords=${encodeURIComponent(keywordsParam)}` : '';
    const categoryQuery = this.categoryName !== 'Danh mục bài viết' && !this.specialtySlug && !kwQuery ? `&category=${encodeURIComponent(this.categoryName)}` : '';
