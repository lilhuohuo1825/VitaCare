...
import { Health } from '../../healthcare/health/health';
import { Remind } from '../remind/remind';
import { AuthService } from '../../../core/services/auth.service';
import { NoticeService, NoticeApiResponse } from '../../../core/services/notice.service';
...
export class Account implements OnInit {
  ...
  private loadNotificationCount(userId: string): void {
    this.noticeService.getNotices(userId).subscribe({
      next: (res: NoticeApiResponse) => {
        setTimeout(() => {
          if (res.success && Array.isArray(res.items)) {
            this.notificationCount = res.items.filter((n: any) => !n.read).length;
          } else {
